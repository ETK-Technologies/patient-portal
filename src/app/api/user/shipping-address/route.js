...
export async function GET(request) {
  try {
    ...
    console.log(
      `[SHIPPING_ADDRESS] Extracted userId: ${userId || "not found"}`
    );
    ...
    if (crmUserIDFromQuery) {
      console.log(
        `[SHIPPING_ADDRESS] Using CRM user ID from query parameter (userData.id): ${crmUserID}`
      );
    } else {
      console.log(
        `[SHIPPING_ADDRESS] Using userId from cookies as CRM user ID: ${crmUserID}`
      );
    }
...
export async function POST(request) {
  try {
    ...
    if (crmUserIDFromQuery) {
      console.log(
        `[SHIPPING_ADDRESS_UPDATE] Using CRM user ID from query parameter (userData.id): ${crmUserID}`
      );
    } else {
      console.log(
        `[SHIPPING_ADDRESS_UPDATE] Using userId from cookies as CRM user ID: ${crmUserID}`
      );
    }
    ...
    console.log(
      `[SHIPPING_ADDRESS_UPDATE] Updating shipping address for user: ${crmUserID}`
    );
  ...
  try {
    ...
    if (!authResult.success) {
      ...
      if (authResult.endpoint) {
        console.error(
          `[SHIPPING_ADDRESS] Failed endpoint: ${authResult.endpoint}`
        );
      }
      return {
        id: crmUserID,
    ...
    console.log(
      `[SHIPPING_ADDRESS] Fetching shipping address from: ${shippingAddressUrl}`
    );
    ...
    console.log(
      "[SHIPPING_ADDRESS] Full CRM response:",
      JSON.stringify(responseData, null, 2)
    );

    console.log(
      `[SHIPPING_ADDRESS] ✓ Successfully fetched shipping address for user: ${crmUserID}`
    );
    return responseData;
  } catch (error) {
    console.error(
      "[SHIPPING_ADDRESS] Error fetching shipping address from CRM:",
      error
    );
    return {
      id: crmUserID,
...
async function updateShippingAddress(crmUserID, updateData) {
  ...
  try {
    ...
    // Use PATCH method with /api/user/shipping-address/update endpoint
    const updateUrl = `${crmHost}/api/user/shipping-address/update`;
    console.log(
      `[SHIPPING_ADDRESS_UPDATE] Updating shipping address at: ${updateUrl}`
    );
    console.log(`[SHIPPING_ADDRESS_UPDATE] User ID: ${crmUserID}`);

    // Map form data to CRM API format
    // Expected format: { id, shipping_address_id, shipping_address_1, shipping_address_2, shipping_city, shipping_state, shipping_country, shipping_postcode }
    const crmUpdateData = {
      id: String(crmUserID), // CRM User ID as string
      shipping_address_id:
        updateData.shipping_address_id || updateData.id || null,
      shipping_address_1:
        updateData.address_1 || updateData.shipping_address_1 || "",
      shipping_address_2:
        updateData.address_2 || updateData.shipping_address_2 || "",
      shipping_city: updateData.city || updateData.shipping_city || "",
      shipping_state:
        updateData.state ||
        updateData.province ||
        updateData.shipping_state ||
        "",
      shipping_country: updateData.country || updateData.shipping_country || "",
      shipping_postcode:
        updateData.postcode ||
        updateData.postal_code ||
        updateData.shipping_postcode ||
        "",
    };

    // Remove null shipping_address_id if not provided
    if (crmUpdateData.shipping_address_id === null) {
      delete crmUpdateData.shipping_address_id;
    }

    console.log(
      `[SHIPPING_ADDRESS_UPDATE] Update data (mapped):`,
      crmUpdateData
    );
    ...
    console.log(
      "[SHIPPING_ADDRESS_UPDATE] Shipping address update response received"
    );
    ...
    console.error(
      "[SHIPPING_ADDRESS_UPDATE] Error updating shipping address:",
      error
    );
    return {
      success: false,

 ...
 * 
 * This function first checks in-memory storage, then falls back to CRM API verification
 * for tokens generated by the main website (which may not be in memory due to serverless cold starts)
 * 
 * @param {string} token - The auto-login token to verify
 * @param {string} wpUserId - Optional WordPress user ID from URL (used for CRM verification fallback)
 */
export async function verifyAutoLoginToken(token, wpUserId = null) {
  console.log(
  ...
  if (token && token.includes("|")) {
    ...
    if (parts.length === 2) {
      userIdFromToken = parts[0];
      tokenToVerify = parts[1];
      console.log(`[TOKEN] Token contains userId prefix: ${userIdFromToken}, extracted token part: ${tokenToVerify.substring(0, 10)}...`);
    }
  }
  ...
  if (global.autoLoginTokens) {
    ...
    if (tokenData) {
      console.log(
        `[TOKEN] Token found in memory! User: ${tokenData.userId}, Expires: ${new Date(
          tokenData.expiresAt
        ).toISOString()}`
      );
    ...
    console.log("[TOKEN] global.autoLoginTokens Map does not exist (cold start?)");
  }

  console.log(`[TOKEN] Token not found in memory, trying CRM API verification...`);
  
  const userIdForVerification = wpUserId || userIdFromToken;
  
  if (userIdForVerification) {
    const verified = await verifyTokenWithCRM(token, userIdForVerification);
    if (verified) {
      console.log(`[TOKEN] Token verified via CRM API for user: ${userIdForVerification}`);
      return userIdForVerification;
    }
  }
  ...
  if (crmHost && token) {
    try {
      ...
      if (verifyResponse.ok) {
        ...
        const verifiedUserId = userData?.user?.wp_user_id || userData?.data?.user?.wp_user_id || userData?.user?.id || userData?.data?.id || userIdFromToken;
        if (verifiedUserId) {
          console.log(`[TOKEN] Token verified as CRM auth token for user: ${verifiedUserId}`);
          return verifiedUserId;
        }
      }
    } catch (error) {
      console.error(`[TOKEN] Error verifying token with CRM:`, error.message);
    }
  }

  console.error(`[TOKEN] Token not found in Map and CRM verification failed!`);
...
/**
 * Verify token with CRM API
 * Used as fallback when token is not found in memory
 * Tries both the full token and the extracted token part
 */
async function verifyTokenWithCRM(token, userId) {
  const crmHost = process.env.CRM_HOST;
  if (!crmHost) {
    return false;
  }
  ...
  if (token && token.includes("|")) {
    const parts = token.split("|");
    if (parts.length === 2) {
      tokensToTry.push(parts[1]);
    }
  }

  for (const tokenToTry of tokensToTry) {
    try {
      const profileUrl = `${crmHost}/api/user/profile?wp_user_id=${userId}`;
      ...
      if (response.ok) {
        ...
        if (data.status || data.success) {
          console.log(`[TOKEN] Token verified with CRM API using ${tokenToTry === token ? 'full token' : 'extracted token part'}`);
          return true;
        }
      }
    } catch (error) {
